    ...
    function handleWheel(y: real, angleDelta: point): void {
        ...
        if (ch?.id === "workspaces" && Config.bar.scrollActions.workspaces) {
            ...
            if (specialWs?.length > 0)
                Hyprland.usingLua ? Hypr.dispatch(`hl.dsp.workspace.toggle_special( "${specialWs.slice(8)}" )`) : Hypr.dispatch(`togglespecialworkspace ${specialWs.slice(8)}`);
            else if (angleDelta.y < 0 || (GlobalConfig.bar.workspaces.perMonitorWorkspaces ? mon.activeWorkspace?.id : Hypr.activeWsId) > 1)
                Hyprland.usingLua ? Hypr.dispatch(`hl.dsp.focus({ workspace = "r${angleDelta.y > 0 ? "-" : "+"}1"})`) : Hypr.dispatch(`workspace r${angleDelta.y > 0 ? "-" : "+"}1`);
